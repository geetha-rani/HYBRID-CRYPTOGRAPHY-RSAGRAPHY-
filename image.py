...
return res
def filenameerror():
messagebox.showinfo('Error Occured!', 'Please select a 8bit/16bit Image
File')
def enfinish():
messagebox.showinfo('Done!', 'Encryption is finished and saved as
EnImage.png')
def definish():
messagebox.showinfo('Done!', 'Decryption is finished and saved as
DeImage.png')
def grey_encrypt():
try:
img = cv.imread(askopenfilename(filetypes = [( "PNG files",
"*.png" ),( "JPEG files", "*.jpeg" ),( "JPG files", "*.jpg" ),("BMP files","*.BMP")]), 0)
#print(type(img))
...
try:
img = cv.imread(askopenfilename(filetypes = [( "PNG files",
"*.png" ),( "JPEG files", "*.jpeg" ),( "JPG files", "*.jpg" ),("BMP files","*.BMP")]))
#print(type(img))
img = img.astype(np.uint16)
a = img.shape
print('\n\nOriginal image: ')
...
def rgb_decrypt():
try:
img1 = imageio.imread(askopenfilename(filetypes = [( "PNG
files", "*.png" )]), format='PNG-FI')
print('\n\nReading Encrypted Image again:\n\n')
print(img1)
img1= img1.tolist()
print('Decrypting....')
